...
routes.post('/create-event', async (req, res) => {
    const { user_id, event_name, event_date, event_hour, description, photo, marker, localization } = req.body;
    if (!user_id || !event_name || !event_date || !event_hour || !description || !photo) {
        ...
        if (marker.length == 0) {
            if (localization) {
                ...
                .then(async (response) => {
                    ...
                    console.log(address);
                    await db.createEvent(user_id, event_name, event_date, event_hour, description, photo, address, 0, 0)
                    return res.status(200).json({ msg: "Evento criado!" })
                })
            } else {
                return res.status(400).json({ msg: "Informe a localização do evento!" })
            }
        } else {
    ...
    try {
        ...
        .then(async (result) => {
            ...
            .then(async (response) => {
                ...
                const address = `${street}, ${neighborhood}, ${city} - ${state}, ${cep}`
                console.log(address);
                await db.updateEvent(event_id, user_id, event_name, event_date, event_hour, description, photo, address, latitude, longitude)
...
routes.get('/get-users-who-go/:id', async (req, res) => {
    const event_id = req.params.id
    try {
        const data = await db.getUsersWhoGo(event_id)
        return res.status(200).json(data)
    } catch (err) {
        return res.status(400).json({ msg: err.message })
    }
